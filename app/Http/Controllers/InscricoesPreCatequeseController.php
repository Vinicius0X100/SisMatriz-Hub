...
use App\Models\User;
use App\Mail\ShareInscricoesPreCatequese;
...
class InscricoesPreCatequeseController extends Controller
{
    public function index(Request $request)
    {
        $query = InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->with('taxa');
        ...
        if ($request->ajax()) {
            return view('modules.inscricoes-pre-catequese.partials.list', compact('records'))->render();
        }

        // Stats for cards
        $stats = [
            'total'    => InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->count(),
            'pending'  => InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->where('status', 0)->count(),
            'approved' => InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->where('status', 1)->count(),
            'rejected' => InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->where('status', 2)->count(),
        ];

        $deadline = PrazoInscricao::where('paroquia_id', Auth::user()->paroquia_id)
            ->where('tipo_inscricao', 'pre_catequese')
            ->first();

        $taxConfig = InscricaoTaxaConfig::where('paroquia_id', Auth::user()->paroquia_id)
            ->where('tipo', 'pre_catequese')
            ->with('items')
            ->first();

        return view('modules.inscricoes-pre-catequese.index', compact('records', 'stats', 'deadline', 'taxConfig'));
    }
    ...
    public function bulkDestroy(Request $request)
    {
        $request->validate([
            'ids'    => 'required|array',
            'ids.*'  => 'exists:inscricoes_pre_catequese,id'
        ]);

        $count = InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)
    ...
    {
        $record = InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)
            ->with('taxa')
            ->findOrFail($id);
    ...
    public function share(Request $request)
    {
        ...
        $records = InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)
            ->whereIn('id', $ids)
            ->orderBy('nome', 'asc')
            ->get();
        ...
        $filename      = 'fichas-pre-catequese-compartilhadas-' . date('d-m-Y') . '.pdf';

        // Send Email
        $users = User::whereIn('id', $request->users)->get();
        foreach ($users as $user) {
            Mail::to($user->email)->send(new ShareInscricoesPreCatequese(
                $senderName,
                $userMessage,
                $inscritosNames,
                $pdfData,
    ...
    private function generatePdfForRecords($records, $isSingle = false, $returnRaw = false)
    {
        ...
        foreach ($records as $record) {
            $fichaPdf = Pdf::loadView('modules.inscricoes-pre-catequese.print', ['records' => [$record]])->output();
            ...
            $attachments = [$record->certidao_batismo];
            ...
            foreach ($attachments as $att) {
                if ($att) {
                    ...
                    if (file_exists($fullPath)) {
                        ...
                        // Image attachments are already rendered in the Blade view
                    }
                }
            }
        }
        ...
        if ($isSingle) {
            ...
            $filename = "ficha-pre-catequese-{$name}.pdf";
        } else {
            $date     = date('d-m-Y');
            $filename = "fichas-pre-catequese-lote-{$date}.pdf";
        }
    ...
    public function export(Request $request)
    {
        ...
        $headers = [
            ...
            "Content-Disposition" => "attachment; filename=inscricoes_pre_catequese_" . date('d-m-Y_H-i') . ".csv",
            "Pragma"              => "no-cache",
        ...
        $columns = [
            ...
            'Telefone 1', 'Telefone 2', 'Endereço', 'Número', 'CEP', 'Estado',
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $record      = InscricaoPreCatequese::where('paroquia_id', Auth::user()->paroquia_id)->findOrFail($id);
