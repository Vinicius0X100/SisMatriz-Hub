...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Nova Apuração Vicentina</h2>
            <p class="text-muted small mb-0">Preencha os dados do assistido/não assistido.</p>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Nova Apuração</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-5">
            ...
            <form action="{{ route('vicentinos-apuracoes.store') }}" method="POST">
                @csrf
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-12">
                         ...
                         <div class="input-group">
                             <input type="text" id="search_person" class="form-control rounded-pill rounded-end-0 bg-light border-0 px-4 py-2" placeholder="Digite o nome ou CPF para pesquisar...">
                             <button type="button" id="search_btn" class="btn btn-primary rounded-pill rounded-start-0 px-4" data-bs-toggle="modal" data-bs-target="#searchModal">
                                 <i class="bi bi-search me-2"></i> Pesquisar
                             </button>
                        </div>
                         <small class="text-muted">Utilize a pesquisa para preencher automaticamente os dados.</small>
                    </div>
                </div>
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control rounded-pill bg-light border-0 px-4 py-2 @error('name') is-invalid @enderror" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="ent_id" id="ent_id" class="form-select rounded-pill bg-light border-0 px-4 py-2 @error('ent_id') is-invalid @enderror" required>
                            ...
                            @foreach($entidades as $entidade)
                                <option value="{{ $entidade->ent_id }}" {{ old('ent_id') == $entidade->ent_id ? 'selected' : '' }}>{{ $entidade->ent_name }}</option>
                            @endforeach
                        </select>
                        @error('ent_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="kind" id="kind" class="form-select rounded-pill bg-light border-0 px-4 py-2 @error('kind') is-invalid @enderror" required>
                            <option value="">Selecione...</option>
                            <option value="1" {{ old('kind') == '1' ? 'selected' : '' }}>Assistido</option>
                            <option value="0" {{ old('kind') == '0' ? 'selected' : '' }}>Não Assistido</option>
                        </select>
                        @error('kind')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-4">
                        ...
                        <select name="month_entire" id="month_entire" class="form-select rounded-pill bg-light border-0 px-4 py-2 @error('month_entire') is-invalid @enderror" required>
                            ...
                            @foreach($meses as $num => $nome)
                                <option value="{{ $num }}" {{ old('month_entire', date('n')) == $num ? 'selected' : '' }}>{{ $nome }}</option>
                            @endforeach
                        </select>
                        @error('month_entire')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="address" id="address" class="form-control rounded-pill bg-light border-0 px-4 py-2 @error('address') is-invalid @enderror" value="{{ old('address') }}">
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-2">
                        ...
                        <input type="text" name="address_number" id="address_number" class="form-control rounded-pill bg-light border-0 px-4 py-2 @error('address_number') is-invalid @enderror" value="{{ old('address_number') }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" class="form-control rounded-4 bg-light border-0 p-4 @error('description') is-invalid @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold">Salvar Apuração</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sincronizar input principal com modal e pré-carregar
    const modalTrigger = document.querySelector('[data-bs-target="#searchModal"]');
    modalTrigger.addEventListener('click', function() {
        if(searchInput.value) {
            modalSearchInput.value = searchInput.value;
            performSearch(searchInput.value);
        } else {
            // Pré-carregar lista se estiver vazio
            performSearch('');
        }
    ...
    function performSearch(query) {
        // Se query vazia, busca os últimos registros (sem filtro)
        let url = `{{ route('vicentinos-apuracoes.search-records') }}`;
        if (query) {
             url += `?q=${encodeURIComponent(query)}`;
        }

        searchResults.innerHTML = '<div class="text-center text-muted py-4"><div class="spinner-border spinner-border-sm text-primary me-2" role="status"></div> Buscando...</div>';
    ...
    function selectPerson(person) {
        ...
        // Atualizar input de pesquisa visualmente
        const searchInput = document.getElementById('search_person');
        const searchBtn = document.getElementById('search_btn');
        
        searchInput.value = person.name;
        searchInput.classList.add('bg-primary', 'bg-opacity-10', 'text-primary', 'fw-bold');
        searchInput.classList.remove('bg-light');
        
        searchBtn.innerHTML = '<i class="bi bi-pencil-square me-2"></i> Mudar';
