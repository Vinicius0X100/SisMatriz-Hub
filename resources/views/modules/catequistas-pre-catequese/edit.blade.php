...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Editar Catequista de Pré-Catequese</h2>
            <p class="text-muted small mb-0">Atualize os dados do catequista.</p>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('catequistas-pre-catequese.index') }}" class="text-decoration-none">Catequistas de Pré-Catequese</a></li>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-5">
            ...
            <form action="{{ route('catequistas-pre-catequese.update', $catequista->id) }}" method="POST">
                @csrf
                @method('PUT')

                <!-- Row 1: Pessoa (Search - pre-filled) -->
                <div class="row g-4 mb-4">
                    <div class="col-md-12">
                        <label for="register_search" class="form-label fw-bold small text-muted">Pessoa (Registros Gerais) <span class="text-danger fw-bold small">(obrigatório)</span></label>
                        <div class="position-relative d-none" id="search_container">
                            ...
                            <input type="hidden" name="register_id" id="register_id" value="{{ $catequista->register_id }}" required>
                            <div id="search_results" class="position-absolute w-100 bg-white shadow-lg rounded-4 mt-2 border p-0" style="z-index: 1000; max-height: 250px; overflow-y: auto; display: none;"></div>
                        </div>
                        <div id="selected_person" class="mt-0">
                            <div class="d-flex align-items-center p-2 bg-primary bg-opacity-10 rounded-3">
                                <i class="bi bi-check-circle-fill text-primary me-2"></i>
                                <span class="fw-bold text-dark" id="selected_person_name">{{ $catequista->nome }}</span>
                            ...
                            <!-- Removed name attribute here to avoid duplication. The value is tracked in #register_id -->
                            <input type="hidden" id="register_id_initial" value="{{ $catequista->register_id }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="ent_id" name="ent_id" required>
                            ...
                            @foreach($entidades as $entidade)
                                <option value="{{ $entidade->ent_id }}" {{ $catequista->ent_id == $entidade->ent_id ? 'selected' : '' }}>{{ $entidade->ent_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="status" name="status" required>
                            <option value="1" {{ $catequista->status == 1 ? 'selected' : '' }}>Ativo</option>
                            <option value="0" {{ $catequista->status == 0 ? 'selected' : '' }}>Inativo</option>
                        </select>
                    </div>
                </div>

                <div class="d-flex justify-content-end gap-2 pt-3">
                    <a href="{{ route('catequistas-pre-catequese.index') }}" class="btn btn-light rounded-pill px-4">Cancelar</a>
