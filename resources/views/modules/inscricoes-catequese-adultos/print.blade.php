...
<head>
    <meta charset="UTF-8">
    <title>Fichas de Inscrição - Catequese de Adultos</title>
    <style>
        @page {
            margin: 0cm 0cm;
        }
...
<body>
    ...
    <div class="header">
        ...
        <h1>Ficha de Inscrição - Catequese de Adultos</h1>
    ...
    @foreach($records as $record)
        <div class="container {{ !$loop->last ? 'page-break' : '' }}">
            ...
            @if($record->certidao_batismo || $record->certidao_primeira_comunhao || $record->certidao_matrimonio)
                <div class="attachments">
                    ...
                    @if($record->certidao_matrimonio)
                        @php
                            $ext = strtolower(pathinfo($record->certidao_matrimonio, PATHINFO_EXTENSION));
                            $matrimonioPath = public_path('storage/uploads/certidoes/' . $record->certidao_matrimonio);
                        @endphp
                        @if(in_array($ext, ['jpg', 'jpeg', 'png', 'webp', 'gif']) && file_exists($matrimonioPath))
                        <div class="attachment-item">
                            <span class="attachment-label">Certidão de Matrimônio</span>
                            <br>
                            <img src="{{ $matrimonioPath }}" alt="Certidão de Matrimônio">
                        </div>
                        @endif
                    @endif
                </div>
