...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <h2 class="mb-0 fw-bold text-dark">Nova Excursão</h2>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <form action="{{ route('excursoes.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-8">
                        <label for="destino" class="form-label fw-bold text-muted small">Destino</label>
                        <input type="text" class="form-control rounded-pill @error('destino') is-invalid @enderror" id="destino" name="destino" value="{{ old('destino') }}" required>
                        @error('destino')
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select rounded-pill @error('tipo') is-invalid @enderror" id="tipo" name="tipo">
                            <option value="excursao" {{ old('tipo') == 'excursao' ? 'selected' : '' }}>Excursão</option>
                            <option value="retiro" {{ old('tipo') == 'retiro' ? 'selected' : '' }}>Retiro</option>
                            <option value="peregrinacao" {{ old('tipo') == 'peregrinacao' ? 'selected' : '' }}>Peregrinação</option>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control rounded-4 @error('descricao') is-invalid @enderror" id="descricao" name="descricao" rows="4">{{ old('descricao') }}</textarea>
                        @error('descricao')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
