                ...
                <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                    ...
                    <div class="d-flex gap-2 mb-3">
                        <button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3" id="selectAllPageBtn">
                            <i class="bi bi-check2-square me-1"></i> Selecionar tudo (página)
                        </button>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-2">
                                <div class="text-muted small">
                                    Parágrafos usados: <span class="fw-bold" id="paragraphCount">0</span>/3
                                </div>
                                <div class="text-danger small d-none" id="paragraphLimitHint">Máximo de 3 parágrafos.</div>
                            </div>
                            <div class="d-flex align-items-start mt-2 text-muted small">
                                <i class="bi bi-info-circle me-2 mt-1"></i>
                                <div>
                                    As variáveis <strong>Nome do Destinatário</strong> e <strong>Seu Nome</strong> serão inseridas automaticamente. Separe sua mensagem em até <strong>3 parágrafos</strong> (uma quebra de linha por parágrafo).
                                </div>
                            </div>
                        </div>
...
<div class="modal fade" id="massCommHelpModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-white border-0 px-4 py-3">
                <div>
                    <h5 class="modal-title fw-bold mb-1">Como escrever a mensagem</h5>
                    <div class="text-muted small">O envio usa um template do WhatsApp com variáveis.</div>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <div class="alert alert-info rounded-4 border-0 shadow-sm mb-3">
                    <div class="d-flex align-items-start">
                        <i class="bi bi-info-circle-fill me-2 mt-1"></i>
                        <div class="small text-muted">
                            O template já inclui automaticamente: <strong>Nome do destinatário</strong> e <strong>Seu nome</strong>.
                            Você só precisa preencher até <strong>3 parágrafos</strong> da mensagem.
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="fw-bold mb-2">Regras</div>
                        <ul class="text-muted small mb-0">
                            <li>Use <strong>uma quebra de linha</strong> para começar o próximo parágrafo.</li>
                            <li>Você pode usar no máximo <strong>3 parágrafos</strong>.</li>
                            <li>Se você tentar criar um 4º parágrafo, ele não será considerado.</li>
                        </ul>
                    </div>
                    <div class="col-md-6">
                        <div class="fw-bold mb-2">Exemplo</div>
                        <div class="bg-light border rounded-4 p-3 small text-muted" style="white-space: pre-line;">
1º parágrafo aqui
2º parágrafo aqui
3º parágrafo aqui
                        </div>
            ...
            <div class="modal-footer bg-white border-top-0 px-4 py-3 justify-content-end gap-2">
                <button type="button" class="btn btn-primary rounded-pill px-4" id="massCommHelpContinue">Continuar</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="selectAllWarningModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        <h5 class="fw-bold mb-2">Atenção ao selecionar tudo</h5>
                        <p class="text-muted small mb-0">
                            Ao usar <strong>Selecionar tudo (página)</strong>, você seleciona todas as pessoas listadas na página atual.
                            Isso pode incluir <strong>qualquer pessoa cadastrada</strong>, e não apenas um grupo específico.
                        </p>
                    </div>
                </div>
            </div>
            <div class="modal-footer bg-white border-top-0 px-4 py-3 justify-content-end gap-2">
                <button type="button" class="btn btn-light border rounded-pill px-4" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-warning rounded-pill px-4" id="confirmSelectAllBtn">Entendi, selecionar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const userId = cfgEl ? (cfgEl.getAttribute('data-user-id') || '') : '';
        let searchTimeout;

        const helpModalEl = document.getElementById('massCommHelpModal');
        const helpContinueBtn = document.getElementById('massCommHelpContinue');
        if (helpModalEl && helpContinueBtn && userId && window.bootstrap) {
            const helpKey = `massComm.helpDismissed.${userId}`;
            const dismissed = window.localStorage.getItem(helpKey) === '1';
            const helpModal = new bootstrap.Modal(helpModalEl);
            if (!dismissed) {
                helpModal.show();
            }
            helpContinueBtn.addEventListener('click', function() {
        ...
        const paragraphLimitHintEl = document.getElementById('paragraphLimitHint');

        function getParagraphsFromValue(value) {
        ...
        function updateParagraphCounter() {
            ...
            const paragraphs = getParagraphsFromValue(messageTextarea.value);
            if (paragraphCountEl) paragraphCountEl.textContent = String(Math.min(paragraphs.length, 3));
            if (paragraphLimitHintEl) paragraphLimitHintEl.classList.toggle('d-none', paragraphs.length <= 3);
        }
        ...
        if (selectAllPageBtn) {
            selectAllPageBtn.addEventListener('click', function() {
                ...
                const warnModalEl = document.getElementById('selectAllWarningModal');
                const confirmBtn = document.getElementById('confirmSelectAllBtn');
                if (warnModalEl && confirmBtn && window.bootstrap) {
                    const warnModal = new bootstrap.Modal(warnModalEl);
                    warnModal.show();
