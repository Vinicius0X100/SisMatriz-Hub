...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <h2 class="mb-0 fw-bold text-dark">Turmas de Pré-Catequese</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Turmas Pré-Catequese</li>
            </ol>
        </nav>
    </div>
        ...
        <div class="card-body p-4">
            ...
            <div class="row g-3 mb-4 align-items-end">
                ...
                <div class="col-md-4 text-end d-flex gap-2 justify-content-end align-items-end">
                    ...
                    <button class="btn btn-primary rounded-pill px-3 d-flex align-items-center justify-content-center gap-2" style="height: 45px;" onclick="window.location.href='{{ route('turmas-pre-catequese.create') }}'">
            ...
            <div id="table-container">
                @include('modules.turmas-pre-catequese.partials.list')
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function fetchData(pageUrl = null) {
        ...
        const url = pageUrl || "{{ route('turmas-pre-catequese.index') }}?" + params.toString();
    ...
    function confirmDelete(id) {
        ...
        confirmBtn.onclick = function() {
            ...
            form.action = "{{ url('turmas-pre-catequese') }}/" + id;
            form.method = 'POST';
    ...
    function openManageModal(id) {
        ...
        fetch(`/turmas-pre-catequese/${id}/students`)
            ...
            .then(data => {
                ...
                document.getElementById('btnApuracaoPresencas').onclick = function() {
                    window.location.href = `/turmas-pre-catequese/${id}/attendance-analysis`;
                };
    ...
    function confirmExport() {
        ...
        if (isBulkExport) {
             ...
             url = `/turmas-pre-catequese/export-bulk?ids=${ids}&type=${type}`;
        } else {
             url = `/turmas-pre-catequese/${currentManageId}/export?type=${type}`;
        }
    ...
    function renderStudents(list) {
        ...
        if(list.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">Nenhum catecúmeno encontrado.</td></tr>';
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (transferForm) {
            transferForm.addEventListener('submit', function(e) {
                ...
                fetch("{{ route('turmas-pre-catequese.transfer') }}", {
                    method: 'POST',
                    body: formData,
                    headers: {
    ...
    async function loadAttendanceList() {
        ...
        try {
            const response = await fetch(`{{ route('turmas-pre-catequese.attendance', ':id') }}`.replace(':id', currentAttendanceTurmaId) + `?date=${date}`);
            ...
            // Clear title first (default for new dates or empty records)
            document.getElementById('attendanceTitle').value = '';
            
            if (existingRecord && existingRecord.title) {
    ...
    function renderAttendanceRows() {
        ...
        if (!currentAttendanceData || currentAttendanceData.students.length === 0) {
             tbody.innerHTML = '<tr><td colspan="4" class="text-center py-5 text-muted">Nenhum catecúmeno(a) nesta turma.</td></tr>';
             return;
        }
    ...
    async function toggleAttendance(studentId, checkbox) {
        ...
        try {
            const response = await fetch(`{{ route('turmas-pre-catequese.attendance.save') }}`, {
                method: 'POST',
                headers: {
    ...
    async function saveAllAttendance() {
        ...
        if (rows.length === 1 && (rows[0].innerText.includes('Carregando') || rows[0].innerText.includes('Selecione uma data') || rows[0].innerText.includes('Nenhum catecúmeno(a)'))) {
            showAlert('warning', 'Carregue a lista de catecúmenos primeiro.');
            return;
        }
        ...
        rows.forEach(row => {
            ...
            if (checkbox) {
                // Assuming toggleAttendance is onchange="toggleAttendance(id, this)"
                // We can extract ID from the function call or better, add data-student-id to the checkbox
                // Let's modify renderAttendanceRows to add data-student-id for reliability
                // Or parse the onclick/onchange attribute. 
                // The current implementation is onchange="toggleAttendance(${student.id}, this)"
        ...
        if (students.length === 0) {
            showAlert('warning', 'Nenhum catecúmeno(a) encontrado.');
            return;
        }
        ...
        try {
            const response = await fetch(`{{ route('turmas-pre-catequese.attendance.save-bulk') }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            if (result.success) {
                showAlert('success', 'Presença de todos os catecúmenos(a) salva com sucesso!');
            ...
            <div class="modal-body p-4">
                ...
                <div id="contentManage" class="d-none">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <h6 class="fw-bold mb-0">Catecúmenos(as)</h6>
                        <div class="d-flex gap-2">
                            ...
                            <div class="position-relative" style="width: 200px;">
                                <input type="text" id="searchStudent" class="form-control form-control-sm rounded-pill ps-5" placeholder="Buscar catecúmeno(a)...">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Transferir Catecúmeno(a)</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
...
<div class="modal fade" id="exportModal" tabindex="-1" aria-hidden="true" style="z-index: 1070;">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            ...
            <div class="modal-body p-4">
                <p class="text-muted mb-4">Selecione o formato para exportar a lista de catecúmenos(a) desta turma.</p
