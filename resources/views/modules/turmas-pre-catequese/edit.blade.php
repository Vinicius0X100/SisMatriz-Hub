...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Editar Turma de Pré-Catequese</h2>
            <p class="text-muted small mb-0">Atualize os dados da turma.</p>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('turmas-pre-catequese.index') }}" class="text-decoration-none">Turmas Pré-Catequese</a></li>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-5">
            <form action="{{ route('turmas-pre-catequese.update', $turma->id) }}" method="POST">
                @csrf
                @method('PUT')
                                    ...
                                    <table class="table table-hover align-middle mb-0">
                                        <thead class="table-light sticky-top">
                                            <tr>
                                                ...
                                                <th class="text-muted small">Origem</th>
                                        ...
                                        <tbody id="selectedStudentsTable">
                                            @forelse($turma->catecandos as $catecando)
                                                <tr id="selected-row-{{ $catecando->register_id }}">
                                                    <td class="ps-3">
                                                        <span class="fw-medium">{{ $catecando->register->name ?? 'Sem nome' }}</span>
                                                        <input type="hidden" name="students[{{ $catecando->register_id }}][id]" value="{{ $catecando->register_id }}">
                                                        <input type="hidden" name="students[{{ $catecando->register_id }}][inscricao_id]" value="{{ $catecando->inscricao_id ?? '' }}">
                                                    </td>
                                                    <td>
                                                        <span class="badge {{ $catecando->inscricao_id ? 'bg-primary bg-opacity-10 text-primary' : 'bg-secondary bg-opacity-10 text-secondary' }} rounded-pill small">
                                                            {{ $catecando->inscricao_id ? 'Inscrição' : 'Registro Geral' }}
                                                        </span>
                                                    </td>
                ...
                <div class="d-flex justify-content-end gap-2 mt-5">
                    <a href="{{ route('turmas-pre-catequese.index') }}" class="btn btn-light rounded-pill px-4">Cancelar</a>
...
<script>
    ...
    async function fetchStudents(query) {
        try {
            ...
            const response = await fetch(`{{ route('turmas-pre-catequese.search-registers') }}?search=${encodeURIComponent(query)}`);
    ...
    window.addStudent = function(id) {
        ...
        row.innerHTML = `
            <td class="ps-3">
                ...
                <input type="hidden" name="students[${id}][id]" value="${student.register_id || id}">
                <input type="hidden" name="students[${id}][inscricao_id]" value="${student.inscricao_id || ''}">
            </td>
            <td>
                <span class="badge ${student.source === 'inscricao' ? 'bg-primary bg-opacity-10 text-primary' : 'bg-secondary bg-opacity-10 text-secondary'} rounded-pill small">
                    ${student.source === 'inscricao' ? 'Inscrição' : 'Registro Geral'}
                </span>
            </td>
            <td class="text-center">
                <div class="form-check d-inline-block">
                    <input class="form-check-input" type="checkbox" name="students[${id}][batizado]" value="1" id="batizado-${id}" ${student.batismo ? 'checked' : ''}>
