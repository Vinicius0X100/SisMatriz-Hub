        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (autoGeneratedPayload.length === 0) {
                    document.getElementById('generateResults').innerHTML = '<div class="alert alert-warning">Nenhuma celebração encontrada ou configurada para esta comunidade no mês.</div>';
                } else {
                    let html = `<p class="text-success fw-bold"><i class="bi bi-check-circle"></i> O algoritmo montou ${autoGeneratedPayload.length} celebrações para a escala.</p>`;
                    html += `<div class="table-responsive" style="max-height: 300px; overflow-y: auto;">
                                <table class="table table-sm table-bordered">
                                    <thead class="table-light sticky-top">
                                        <tr><th>Dia</th><th>Hora</th><th>Acólitos Distribuídos</th></tr>
                                    </thead>
                                    <tbody>`;
                    autoGeneratedPayload.forEach(cel => {
                        html += `<tr>
                                    <td>${cel.data}</td>
                                    <td>${cel.hora}</td>
                                    <td>${cel.acolitos.length} pessoa(s)</td>
                                 </tr>`;
                    });
