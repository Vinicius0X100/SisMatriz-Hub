...
<script>
    ...
    function publishGenerate() {
        if(!confirm('Deseja realmente aprovar todas essas celebrações? Elas irão direto para a escala oficial e as mensagens de WhatsApp serão enviadas!')) return;

        const btn = document.getElementById('btnPublishGenerate');
        btn.disabled = true;
        btn.innerHTML = 'Publicando e Notificando...';
        ...
        .then(response => {
            if (response.ok) {
                ...
                btn.disabled = false;
                btn.innerHTML = 'Aprovar Todos (WhatsApp)';
            }
        })
        .catch(error => {
            console.error(error);
        ...
        <div class="modal-content border-0 shadow rounded-4">
            ...
            <div class="modal-footer border-top-0 pt-0 justify-content-between">
                ...
                <div class="d-flex gap-2">
                    ...
                    <button type="button" id="btnPublishGenerate" class="btn btn-success rounded-pill px-4 shadow-sm" style="display: none;" onclick="publishGenerate()"><i class="bi bi-whatsapp me-2"></i>Aprovar Todos</button>
                </div>
            </div>
        </div>
    </div>
