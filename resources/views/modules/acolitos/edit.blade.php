...
@section('title', 'Editar Acólito/Coroinha')

@section('content')
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Editar Acólito/Coroinha</h2>
            <p class="text-muted small mb-0">Atualize os dados do membro da equipe de liturgia.</p>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-5">
            <form action="{{ route('acolitos.update', $acolito->id) }}" method="POST" id="editAcolitoForm">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="hidden" name="register_id" id="hiddenInput" value="{{ $acolito->register_id }}">
                    @error('register_id')
                ...
                <div class="row g-4 mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="ent_id" name="ent_id" required>
                            ...
                            @foreach($entidades as $entidade)
                                <option value="{{ $entidade->ent_id }}" {{ $acolito->ent_id == $entidade->ent_id ? 'selected' : '' }}>{{ $entidade->ent_name }}</option>
                            @endforeach
                        </select>
                        @error('ent_id')
                            <div class="text-danger small mt-1 ps-3 fw-bold">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="type" name="type" required>
                            <option value="">Selecione...</option>
                            <option value="0" {{ $acolito->type == 0 ? 'selected' : '' }}>Acólito</option>
                            <option value="1" {{ $acolito->type == 1 ? 'selected' : '' }}>Coroinha</option>
                        </select>
                        @error('type')
                            <div class="text-danger small mt-1 ps-3 fw-bold">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control rounded-pill bg-light border-0 px-4 py-2" id="graduation_year" name="graduation_year" value="{{ $acolito->graduation_year }}" placeholder="Ex: 2023" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="status" name="status" required>
                            <option value="0" {{ $acolito->status == 0 ? 'selected' : '' }}>Ativo</option>
                            <option value="1" {{ $acolito->status == 1 ? 'selected' : '' }}>Inativo</option>
                        </select>
                        @error('status')
                            <div class="text-danger small mt-1 ps-3 fw-bold">{{ $message }}</div>
                        @enderror
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const initialName = "{{ $acolito->register->name ?? $acolito->name ?? '' }}";
        const initialId = "{{ $acolito->register_id }}";
        
        if (initialId && initialName) {
            selectPerson(initialId, initialName);
        }
    });
