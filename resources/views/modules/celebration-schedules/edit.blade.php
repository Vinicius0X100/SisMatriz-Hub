...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <div>
            <h2 class="mb-0 fw-bold text-dark">Editar Horário</h2>
            <p class="text-muted small mb-0">Atualize as informações do horário de celebração.</p>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-5">
            <form action="{{ route('celebration-schedules.update', $record->id) }}" method="POST" id="editForm">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select rounded-pill bg-light border-0 px-4 py-2" id="dia_semana" name="dia_semana" required>
                            ...
                            @foreach($diasSemana as $dia)
                                <option value="{{ $dia }}" {{ old('dia_semana', $record->dia_semana) == $dia ? 'selected' : '' }}>{{ $dia }}</option>
                            @endforeach
                        </select>
                        @error('dia_semana')
                            <div class="text-danger small mt-1 ps-3 fw-bold">{{ $message }}</div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="time" class="form-control rounded-pill bg-light border-0 px-4 py-2" id="horario" name="horario" required value="{{ old('horario', $record->horario) }}">
                        @error('horario')
                            <div class="text-danger small mt-1 ps-3 fw-bold">{{ $message }}</div>
                        @enderror
                    </div>
