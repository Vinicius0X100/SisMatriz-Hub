...
<div class="container-fluid px-4">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <form action="{{ route('turmas-adultos.attendance-analysis', $turma->id) }}" method="GET" class="row g-3" id="filterForm">
                <div class="col-md-7">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    <thead class="bg-light">
                        <tr>
                            <th scope="col" class="border-0 rounded-start ps-4">Nome do Catecando(a)</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-sm btn-outline-success rounded-pill px-3 fw-bold" 
                                        onclick="showDetailsModal('Presenças - {{ addslashes($student['name']) }}', {{ htmlspecialchars(json_encode($student['presencas_list']), ENT_QUOTES, 'UTF-8') }}, 'success')">
                                    {{ $student['presencas'] }}
                                </button>
                            </td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-outline-danger rounded-pill px-3 fw-bold"
                                        onclick="showDetailsModal('Faltas - {{ addslashes($student['name']) }}', {{ htmlspecialchars(json_encode($student['faltas_list']), ENT_QUOTES, 'UTF-8') }}, 'danger')">
                                    {{ $student['faltas'] }}
                                </button>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-end pe-4">
                                <a href="{{ route('turmas-adultos.attendance-history', ['id' => $turma->id, 'student_id' => $student['id']]) }}" class="btn btn-primary btn-sm rounded-pill px-3">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                ...
                                Nenhum catecando encontrado nesta turma.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
