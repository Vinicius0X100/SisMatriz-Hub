...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
        <h2 class="mb-0 fw-bold text-dark">Turmas de Catequese de Adultos</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Turmas de Adultos</li>
            </ol>
        </nav>
    </div>
        ...
        <div class="card-body p-4">
            ...
            <div class="row g-3 mb-4 align-items-end">
                ...
                <div class="col-md-4 text-end d-flex gap-2 justify-content-end align-items-end">
                    ...
                    <button class="btn btn-primary rounded-pill px-3 d-flex align-items-center justify-content-center gap-2" style="height: 45px;" onclick="window.location.href='{{ route('turmas-adultos.create') }}'">
            ...
            <div id="table-container">
                @include('modules.turmas-adultos.partials.list')
            </div>
        </div>
    </div>
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="contentManage" class="d-none">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            <h6 class="fw-bold mb-0">Lista de Catecandos(as)</h6>
                            <div class="d-flex gap-2">
                                ...
                                <div class="position-relative" style="width: 200px;">
                                    <input type="text" id="searchStudent" class="form-control form-control-sm rounded-pill ps-4" placeholder="Buscar catecando(a)...">
    ...
    <div class="modal fade" id="transferModal" tabindex="-1" aria-hidden="true" style="z-index: 1060;">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-body p-4">
                    <p class="mb-4">
                        Você está transferindo o catecando(a) <strong id="transferStudentName" class="text-primary"></strong>.
    ...
    <div class="modal fade" id="exportModal" tabindex="-1" aria-hidden="true" style="z-index: 1070;">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0 shadow">
                ...
                <div class="modal-body p-4">
                    <p class="text-muted mb-4">Selecione o formato para exportar a lista de catecandos desta turma.</p>
...
<script>
    ...
    function fetchData(pageUrl = null) {
        ...
        const url = pageUrl || "{{ route('turmas-adultos.index') }}?" + params.toString();
    ...
    function restoreSelection() {
        ...
        // Restore Select All if all are checked
        const selectAll = document.getElementById('selectAll');
        if (selectAll) {
            const checkboxes = Array.from(tableContainer.querySelectorAll('.row-checkbox'));
    ...
    function updateBulkActionsUI() {
        ...
        if (btn) {
            ...
            btn.innerHTML = count > 0 ? `Ações (${count})` : 'Ações em Massa';
        }
    }
    ...
    function confirmDelete(id) {
        ...
        confirmBtn.onclick = function() {
            ...
            form.action = "{{ url('turmas-adultos') }}/" + id;
            form.method = 'POST';
    ...
    function bulkDelete() {
        if(confirm('Tem certeza que deseja excluir as turmas selecionadas?')) {
             // Implementation for bulk delete would go here
             // Typically posting to a route that handles array of IDs
             console.log('Deleting: ', Array.from(selectedIds));
             alert('Funcionalidade de exclusão em massa a ser implementada no controller.');
        }
    }
    ...
    function openManageModal(id) {
        ...
        fetch(`/turmas-adultos/${id}/students`)
            ...
            .then(data => {
                ...
                document.getElementById('btnApuracaoPresencas').onclick = function() {
                    window.location.href = `/turmas-adultos/${id}/attendance-analysis`;
                };
    ...
    function confirmExport() {
        ...
        if (isBulkExport) {
             ...
             url = `/turmas-adultos/export-bulk?ids=${ids}&type=${type}`;
        } else {
             url = `/turmas-adultos/${currentManageId}/export?type=${type}`;
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (transferForm) {
            transferForm.addEventListener('submit', function(e) {
                ...
                fetch("{{ route('turmas-adultos.transfer') }}", {
                    method: 'POST',
                    body: formData,
                    headers: {
    ...
    async function loadAttendanceList() {
        ...
        try {
            const response = await fetch(`{{ route('turmas-adultos.attendance', ':id') }}`.replace(':id', currentAttendanceTurmaId) + `?date=${date}`);
    ...
    async function toggleAttendance(studentId, checkbox) {
        ...
        try {
            const response = await fetch(`{{ route('turmas-adultos.attendance.save') }}`, {
                method: 'POST',
                headers: {
    ...
    async function saveAllAttendance() {
        ...
        try {
            const response = await fetch(`{{ route('turmas-adultos.attendance.save-bulk') }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            if (result.success) {
                showAlert('success', 'Presença de todos os catecandos salva com sucesso!');
